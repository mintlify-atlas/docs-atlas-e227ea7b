...
description: "Complete reference for all environment variables in the Debuta backend .env file, including MongoDB, JWT, Cloudinary, email, CORS, and social OAuth settings."
---
...
<ParamField path="MONGO_URI" type="string" required>
  ...
  The server uses a linear backoff retry loop (up to 5 attempts, 3 s apart) when the initial connection fails, so transient DNS or network issues at startup are handled gracefully.
</ParamField>
